...
def bin_and_interpolate(s, normflux, bins, sigma, inmask, plot=False):
    ...
    # interp = interpolate.interp1d(knots, bin_means, bounds_error=False, fill_value='extrapolate')

    # knts = np.arange(s_srtd[0], s_srtd[-1], 0.4)[1:]
    knts = np.linspace(s_srtd[0], s_srtd[-1], bins)[1:-1]

    ti, c, k = interpolate.splrep(s_srtd, normflux_srtd, t=knts, k=1)
    interp = interpolate.BSpline(ti, c, k)

    return interp, outlier_mask
...
def correct(time, flux, centroid_col, centroid_row, breaks, tr_mask=None, polyorder=5, niters=3, bins=15, sigma_1=3.,
            sigma_2=5., progress=True, restore_trend=False, plot_arcs=False, plot_windows=False):
    """
...
if __name__ == "__main__":
    ...
    fc, tr = correct(t, f, x*4., y*4., break_points, None, niters=4, bins=12,
                     # plot_windows=True,
                     # plot_arcs=True
                     )
