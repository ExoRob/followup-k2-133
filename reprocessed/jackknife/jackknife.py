...
class Planet(object):
    ...
    def set_priors(self):
        ...
        self.prior_rp_01 = stats.norm(prior_rp, width_rp)    # planet/star radius ratio
        self.prior_inc_01 = stats.uniform(prior_i, width_i)  # orbital inclination (degrees)
        self.prior_t0_01 = stats.norm(prior_t0, width_t0)    # time of inferior conjunction
        self.prior_period_01 = stats.norm(prior_per, width_per)  # orbital period (days)
...
class Data(object):
    ...
    def __init__(self, lc_file, skip_lc_rows=0, cut=None):
        ...
        if cut:
            ...
            self.LCerror = np.delete(self.LCerror, cut)
...
prior_t0, width_t0 = 3004.8659, 0.01
prior_per, width_per = 26.5837, 0.005
prior_rp, width_rp = 0.0372, 0.005
prior_i, width_i = 88.8, 1.

out_dir = "jackknife-poly/"

ncores = 20
ndim, nwalkers, nsteps, burnin = 4, 400, 5000, 3000

lcname = "lc-poly.dat" # "lc-mySFF-cut-transits.dat"
...
def mcmc_cut(ind):
    ...
    out = sampler.run_mcmc(pos, nsteps, progress=False)
    ...
    # plt.savefig(out_dir + str(ind) + "_samples_all.pdf", format="pdf")
    # # plt.show()
    # plt.close()
    ...
    for pc in pcs:
        ...
        # save_str += (",".join(vals) + "\n")
        save_str += (str(pc) + "\n")
    with open(out_dir + str(ind) + "_fit_vals.dat", "w") as save_file:
    ...
    print ind, "/", t_full.size, "-", np.asarray(['%9.4f' % s for s in median_pars]), round(b, 3), round(rp, 3), \
        round((t2 - t1) / 60., 1)
    ...
    # plt.tight_layout()
    # plt.savefig(out_dir + str(ind) + "_fit.pdf", format="pdf")
    # # plt.show()
    # plt.close()

    with open(out_dir + str(ind) + "_mcmc.pkl", "wb") as pklf:
        dill.dump([data, planet, samples], pklf)


pool = multiprocessing.Pool(processes=ncores)
pool.map(mcmc_cut, range(t_full.size))
