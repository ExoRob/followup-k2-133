...
width_per = [0.001, 0.001, 0.002, 0.005]
prior_rp = [0.0255, 0.0288, 0.0393, 0.0372]
width_rp = [0.005, 0.005, 0.005, 0.005]
prior_i = [86., 87., 88., 88.8]
width_i = [4., 3., 2., 1.2]
...
def single_model(pars, t):
    """ return the transit curve for one planet at times t """

    rp, inc, t0, per, a_h, a_r, a_t = pars

    c2, c3, c4 = LDC3.forward([a_h, a_r, a_t])

    params = batman.TransitParams()
    params.limb_dark = "nonlinear"  # limb darkening model
    params.u = [0., c2, c3, c4]     # limb darkening coefficients
    params.ecc = 0.
    params.w = 0.
...
class Planet(object):
    def __init__(self):
        # fixed parameters
        self.limb_dark = "nonlinear"    # limb darkening model
        ...
        self.alpha_h = 0.            # limb darkening coefficients
        self.alpha_r = 0.
        self.alpha_t = 0.
        
        self.stellar_radius, self.stellar_mass = st_r, st_m

        self.N_free_parameters = sum(pl_use) * 4 + 3    # number of free parameters in model

        # start the batman model
        self.params = batman.TransitParams()
        self.params.limb_dark = self.limb_dark  # limb darkening model
        # self.params.u = []               # limb darkening coefficients
        ...
        self.labels += [r'$\alpha_h$', r'$\alpha_r$', r'$\alpha_\theta$']
    ...
    def set_transit_parameters(self, pars):
        ...
        if pl_use[3]:
            self.rp_01, self.inc_01, self.t0_01, self.period_01 = pars[12:16]

        self.alpha_h, self.alpha_r, self.alpha_t = pars[-3:]
    ...
    def get_transit_curve(self, t):
        ...
        if all((0. < v < 1.) for v in [self.alpha_h, self.alpha_r, self.alpha_t]):      # if LDCs OK
            c2, c3, c4 = LDC3.forward([self.alpha_h, self.alpha_r, self.alpha_t])
            self.params.u = [0., c2, c3, c4]
        else:
            # self.params.u = [0.5, 0.1, 0.1, -0.1]
            return np.ones(t.size) * np.nan     # model not valid
    ...
    def set_priors(self):
        ...
        # [0.4475975539817997, 0.7294067670007804, 0.6557480380299435]
        # self.prior_alpha_h = stats.uniform(0.4, 0.1)
        # self.prior_alpha_r = stats.uniform(0.67, 0.1)
        # self.prior_alpha_t = stats.uniform(0.6, 0.1)

        self.prior_alpha_h = stats.uniform(0., 1.)
        self.prior_alpha_r = stats.uniform(0., 1.)
        self.prior_alpha_t = stats.uniform(0., 1.)
    ...
    def get_from_prior(self, nwalkers):
        ...
        pfp += [self.prior_alpha_h.rvs(nwalkers), self.prior_alpha_r.rvs(nwalkers), self.prior_alpha_t.rvs(nwalkers)]
...
class Data(object):
    ...
    def __init__(self, lc_file, skip_lc_rows=0):
        ...
        with open("pars.pkl", "rb") as pf:
            pars = dill.load(pf)
        m_all = np.ones(self.LC.size, float)
        for i, params in enumerate(pars):
            bat = batman.TransitModel(params, self.LCtime, supersample_factor=15, exp_time=29.4 / 60. / 24.)
            m = bat.light_curve(params)
            m_all += (m - 1.)

        resid = self.LC - m_all
        mask = resid < 5. * self.LCerror

        # plt.plot(self.LCtime[mask], self.LC[mask], "k.")
        # plt.plot(self.LCtime[~mask], self.LC[~mask], "rx")
        # plt.show()
...
def lnlike(pars, planet):
    ...
    if all(0. < v < 1. for v in pars[-3:]):
        alphas_passed = LDC3.criteriatest(0, LDC3.forward(pars[-3:]))
    else:
        alphas_passed = False

    if (not np.isfinite(log_like)) or (not alphas_passed):
        return -np.inf
    else:
        return log_like
...
def lnprior(pars, planet):
    ...
    prior_alpha_h = planet.prior_alpha_h.logpdf(pars[16])
    prior_alpha_r = planet.prior_alpha_r.logpdf(pars[17])
    prior_alpha_t = planet.prior_alpha_t.logpdf(pars[18])
    ln_prior_alpha = prior_alpha_h + prior_alpha_r + prior_alpha_t

    ln_prior += ln_prior_alpha

    # print pars[-3:], prior_alpha_h, prior_alpha_r, prior_alpha_t, ln_prior_alpha, ln_prior
    # print [type(thing) for thing in [pars[-3], pars[-2], pars[-1],
    #                                  prior_alpha_h, prior_alpha_r, prior_alpha_t, ln_prior_alpha, ln_prior]]
...
data = Data("final-lc-mySFF.dat")

# parameters for emcee
# ndim, nwalkers, nsteps, burnin = planet.N_free_parameters, 60, 50, 20      # testing
# ndim, nwalkers, nsteps, burnin = planet.N_free_parameters, 100, 500, 300      # basic
# ndim, nwalkers, nsteps, burnin = planet.N_free_parameters, 250, 1500, 800    # fitting
ndim, nwalkers, nsteps, burnin = planet.N_free_parameters, 300, 4000, 2600   # fitting

out_dir = "save_{}_{}_{}_{}_{}/".format("broad", ndim, nwalkers, nsteps, burnin)
...
for i in range(planet.N_free_parameters):
    ...
    plt.plot(sampler.chain[0,:,i])
    plt.axvline(burnin, color="r", ls="--")
    plt.savefig(out_dir + planet.labels[i].replace('$', '').replace('_', '').replace('/', '').replace(' ', '')
                + "_convergence.pdf")
    plt.close()
...
print(np.asarray(['%9.4f' % s for s in median_pars]))
pcs = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples,
                                                                     [50.-68.27/2., 50, 50.+68.27/2.], axis=0)))
...
for i in range(4):
    if pl_use[[2, 1, 0, 3][i]]:
        ...
        if i == 3:
            pack = ["01", planet.period_01, planet.t0_01, planet.rp_01, planet.inc_01, median_pars[12:16], [12, 16]]
        pl, period, t0, rp, inc, pars, inds = pack
        pars = np.append(pars, median_pars[-3:])
