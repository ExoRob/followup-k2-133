...
def injection_test(ind):
    ...
    m, pars, depth, ntrans, ndp, injphase, impact = \
        random_transit_model(t, per_range, (.7e-4, 6e-3), (400e-6, 1400e-6), (3, 100))

    f_i = f + m - 1.    # injected flux

    f_i, tr = sff_detrender.correct(t, f_i, x*4., y*4., break_points, None, niters=4, bins=12, progress=False)
    ...
    return ind, recovered, depth, sde, ndp, pars, ntrans, injphase, impact, vals
...
n_tests = 10000     # number of injections to do
...
rec_ar, depth_ar, sde_ar, dp_ar, pars_ar, ntrans_ar, phase_ar, impact_ar, vals_ar = \
    np.array([], dtype=int), np.array([], dtype=int), np.array([], dtype=float), np.array([], int), np.array([]), \
    np.array([], int), np.array([]), np.array([]), np.array([])
...
for res in tqdm(pool.imap_unordered(injection_test, range(n_tests)), total=n_tests, desc="Running injection tests"):
    ...
    vals_ar = np.append(vals_ar, res[9])
...
with open("inj-res{}.pkl".format(run), "wb") as pf:
    dill.dump([rec_ar, depth_ar, sde_ar, pars_ar, dp_ar, ntrans_ar, phase_ar, impact_ar, vals_ar, param_dists], pf)
