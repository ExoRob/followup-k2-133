...
rec_ar, depth_ar, sde_ar, pars_ar, param_dists = np.array([]), np.array([]), np.array([]), np.array([]), [[]]*6
dp_ar, ntrans_ar, phase_ar, impact_ar = np.array([], int), np.array([]), np.array([]), np.array([])
vals_ar = np.array([])
for i in range(0, 6):
...
pers = np.asarray(param_dists[0])
t0s = np.asarray(param_dists[1])
...
# for i in tqdm(range(1000)):
    ...
    if period >= 1. and 2988.5416652958083 <= t0 < 2988.5416652958083+100.:      # set T0 equal to first epoch
        while t0-period > 2988.5416652958083:
            t0 -= period

    t_ss = np.linspace(t0s[i]-pers[i]/2., t0s[i]+pers[i]/2., 10000)     # one orbit
    ...
    dur = float(inds_in_transit[-1] - inds_in_transit[0]) * (t_ss[-1] - t_ss[0]) / 10000.    # transit duration in days
...
ax1.plot(pers[rec_ar], depth_ar[rec_ar], "b.", ms=1, alpha=0.99, zorder=1)     # recovered
ax1.plot(pers[~rec_ar], depth_ar[~rec_ar], "r.", ms=1, alpha=0.99, zorder=1)   # missed

for i in range(4):
    dep, per = [741.0, 1103., 1936., 1009.][i], [3.0714, 4.8678, 11.0243, 26.5837][i]
    if dep < 1390.:
        ax1.plot(per, dep, marker="*", ls="", ms=20, color="k", zorder=2)
        ax1.plot(per, dep, marker="*", ls="", ms=10, color="g", zorder=3)
    else:
...
for i in range(dbins.size - 1):         # each row (from top down)
    ...
    for j in range(pbins.size - 1):     # each column
        ...
        rec = rec_ar[(pers >= p1) & (pers < p2) & (depth_ar >= d1) & (depth_ar < d2)]   # recovered array
...
#     dill.dump([rec_ar, im, pbins, dbins], pf)

# img = ax1.imshow(im, extent=(pbins.min(), pbins.max(), dbins.min(), dbins.max()),
#                  aspect="auto", cmap=c, alpha=0.5, vmax=1., vmin=0.)    # vmin=np.nanmin(im, axis=(1, 0))
# fig.colorbar(img, cax=ax2)

# cset = plt.contour(x, y, gaussian_filter(im, 3.5)*100, np.arange(0., 100., 10.),
...
cont = ax1.contourf(np.linspace(pbins[0], pbins[-1], pbins.size-1), np.linspace(dbins[0], dbins[-1], dbins.size-1),
                    gaussian_filter(im, 2.2)*100., np.linspace(0., 100., 10000),
                    antialiased=False, cmap="coolwarm_r", alpha=1, zorder=0)
