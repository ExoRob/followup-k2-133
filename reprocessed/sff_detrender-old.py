...
if __name__ == "__main__":
    ...
    # break_points = np.arange(0, t.size, 250)[1:]
    ...
    fc, tr, s, tr1, tr2 = correct(t, f, x*4., y*4., break_points, tmask, niters=14,
                                    # method="spline",
                                    # plot_windows=True,
                                    # plot_arcs=True,
                                    # restore_trend=True,
    ...
    print("6.5-hr CDPP = {:.0f} ppm".format(cdpp*1e6))

    # plt.hist(res*1e6, bins=np.linspace(-2e3, 2e3, 200))
    # plt.xlim(res.min()*1e6, res.max()*1e6)
    # plt.show()
    ...
    markers, caps, bars = \
        ax.errorbar(bins, rms, [rmsl, rmsh], color="k", lw=1., label="K2 data", zorder=2, elinewidth=2., ecolor="0.5")
