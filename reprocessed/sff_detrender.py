...
if __name__ == "__main__":
    ...
    # lc = fits.open("../transit/ktwo247887989-c13_llc.fits")[1].data
    ...
    # plt.plot(t[q == 0], f[q == 0], ".")
    # # plt.plot(t[fmask], f[fmask], "ko")
    # _ = [plt.axvline(v) for v in [2988.5, 3064.7]]
    # plt.show()
    ...
    # n_samples = 2000
    # time_knots_sample = np.random.uniform(1., 15., n_samples)
    # rot_knots_sample = np.random.uniform(0.05, 2., n_samples)
    # arc_knots_sample = np.random.uniform(0.05, 2., n_samples)
    # res_ar, ph_res_ar = [], []
    # for l in tqdm(range(n_samples)):
    #     try:
    #         fc, tr, s = correct(t, f, x*4., y*4., break_points, tmask, niters=4, method="spline", progress=False,
    #                             time_knotspacing=time_knots_sample[l],
    #                             rot_knotspacing=rot_knots_sample[l],
    #                             arc_knotspacing=arc_knots_sample[l])
    #         res = fc - m_4
    #         phase = ((t-pars[3].t0+pars[3].per/2.0) / pars[3].per) % 1
    #         phase = phase[tmask]
    #         ph_err = np.std(fc[tmask][(phase >= 0.495) & (phase <= 0.505)]) * 1e6
    #         err = np.median([np.std(res[i:i + 13]) for i in range(0, len(res), 13)]) * 1e6
    #         ph_res_ar.append(ph_err)
    #         res_ar.append(err)
    #     except ValueError:
    #         ph_res_ar.append(np.nan)
    #         res_ar.append(np.nan)
    #     # print res_ar[l], "-", time_knots_sample[l], rot_knots_sample[l], arc_knots_sample[l]
    # with open("test_knots.pkl", "wb") as pf:
    #     dill.dump([n_samples, time_knots_sample, rot_knots_sample, arc_knots_sample, res_ar, ph_res_ar], pf)
    # import sys; sys.exit()

    # print [pars[3].t0 + i*pars[3].per for i in range(3)]
    ...
    plt.plot(t, tr)
    _ = [plt.axvline(v) for v in np.arange(3)*pars[3].per + pars[3].t0]
    plt.show()
    ...
    # t_int = lc.TIME[(lc.TIME > t[0]) & (lc.TIME < t[-1])]
    # res_int = interpolate.interp1d(t, res)(t_int)
    # res = res_int

    # gap_mask = (np.arange(t.size) > 27) & (np.arange(t.size) < 3293)
    # t, res = t[gap_mask], res[gap_mask]
    # plt.plot(t_int, res_int, "r.")
    # plt.plot(t, res, "k.")
    # plt.plot((t[1:] - t[:-1]) / (29.4 / 60. / 24.) - 1)
    # plt.show()
    # res = res[:-1]
    # res = res.reshape(res.size / 2, 2).mean(axis=1)

    # dist = norm
    # bins = np.linspace(-1e3, 1e3, 150)
    # mu, std = dist.fit(res*1e6)
    # p = dist.pdf(bins, mu, std)

    # plt.figure(figsize=(8, 6))
    # plt.hist(res*1e6, bins=bins, density=True, color="k", histtype="stepfilled")
    # plt.plot(bins, p, 'r', linewidth=2)
    # # plt.plot(np.linspace(-2e3, 2e3, 5000), gaus(np.linspace(-2e3, 2e3, 5000), popt[0], popt[1], popt[2]))
    # plt.xlim(-750, 750)
    ...
    # with open("rednoise.pkl", "wb") as pf:
    #     dill.dump([res, t, lc.TIME], pf)

    # fig, ax = plt.subplots(1, 1, figsize=(7, 5))
    # bns, rmss, rmsg, rmsse = [], [], [], []
    # for bn in range(1, 1500):
    #     cut = res.size % bn
    ...
    # plt.tight_layout(0.)
    # plt.savefig("my-code.pdf")
    # plt.show()
    # plt.clf()
    #
    # rms, rmsl, rmsh, stderr, bins = mc3.rednoise.binrms(res, 1500, 1)
    ...
    #
    # ax.plot(bins, rms, c="k", label="K2 data")
    # plt.fill_between(bins, rms-rmsl, rms+rmsh, where=(rms+rmsh) > (rms-rmsl), interpolate=True, facecolor="0.5",
    ...
    # plt.tight_layout(0.)
    # plt.savefig("red-noise-binning.pdf", dpi=200)
    # # plt.show()
    # plt.clf()
